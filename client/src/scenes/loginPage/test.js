  ...
  async function addCountryCode() {
    ...
    for (var j = 0; j < countries.length; j++) {
        for (const [key, value] of Object.entries(data)) {
            if (value === countries[j].name){
                //console.log(countries[j].name, " ", value, " ", key)
                countries[j].code = key;
            }
          }


      //const code = await getCountryCode(countries[i].name);
    ...
    //console.log(countries);
      ...
      let filteredList = countries.filter(function(obj) {
        return obj.hasOwnProperty('code')
      });
      
      console.dir(filteredList,  {'maxArrayLength': null});
